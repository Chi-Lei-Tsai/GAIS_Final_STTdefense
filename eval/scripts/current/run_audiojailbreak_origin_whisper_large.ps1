param(
  ...
  [string]$GuardResults = "eval\results\structured_provenance_audiojailbreak_origin_whisper_large.jsonl",
  [string]$PassedManifest = "eval\manifests\audiojailbreak_origin_guard_passed_whisper_large.jsonl",
  [string]$NemotronResults = "eval\results\nvidia_api_baselines\nemotron_audiojailbreak_origin_guard_passed_whisper_large.jsonl",
  [string]$JudgeResults = "eval\results\nvidia_api_baselines\nemotron_audiojailbreak_origin_guard_passed_lg4_whisper_large.jsonl",
  [string]$AudioJailbreakRoot = "",
  [string]$AsrModel = "openai/whisper-large-v3",
...
if ($AudioJailbreakRoot) {
  $manifestArgs += @("--local-repo-root", $AudioJailbreakRoot)
}
& $Python @manifestArgs
Assert-NativeStepSucceeded "Build/resume full AudioJailbreak Origin manifest"

Write-Host "[2/5] Run structured provenance guard with Whisper-large"
& $Python -u `
