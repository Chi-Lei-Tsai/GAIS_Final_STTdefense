param(
  ...
  [string]$GuardResults = "eval\results\structured_provenance_audiojailbreak_origin_full_pi_guard_routed_small_large_cpu.jsonl",
  [string]$PassedManifest = "eval\manifests\audiojailbreak_origin_full_guard_passed_for_nemotron_routed_small_large_cpu.jsonl",
  [string]$NemotronResults = "eval\results\nvidia_api_baselines\nemotron_audiojailbreak_origin_guard_passed_full_audio_routed_small_large_cpu.jsonl",
  [string]$JudgeResults = "eval\results\nvidia_api_baselines\nemotron_audiojailbreak_origin_guard_passed_lg4_context_routed_small_large_cpu.jsonl",
  [string]$DefaultAsrModel = "openai/whisper-small",
  [string]$OverlapAsrModel = "openai/whisper-large-v3",
...
Write-Host "[2/5] Run structured provenance guard with diarization-routed ASR on CPU"
& $Python -u `
  eval\scripts\current\run_structured_provenance_incremental.py `
  --manifest $Manifest `
  --out $GuardResults `
  --asr-model $DefaultAsrModel `
  --overlap-asr-model $OverlapAsrModel `
