param(
  ...
  [string]$GuardResults = "eval\results\structured_provenance_audiojailbreak_origin_full_pi_guard_asr440.jsonl",
  [string]$PassedManifest = "eval\manifests\audiojailbreak_origin_full_guard_passed_for_nemotron.jsonl",
  [string]$NemotronResults = "eval\results\nvidia_api_baselines\nemotron_audiojailbreak_origin_guard_passed_full_audio.jsonl",
  [string]$JudgeResults = "eval\results\nvidia_api_baselines\nemotron_audiojailbreak_origin_guard_passed_lg4_context.jsonl"
)
...
Write-Host "[2/5] Run structured provenance guard"
& $Python -u `
